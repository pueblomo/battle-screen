...
export default function Feature({
  ...
  const filteredSpells: MonsterSpellcasting[] | undefined = spells?.filter(
    (spell) => spell.displayAs === display,
  );

  function styleEntry(entry: Entry, index: number): ReactNode {
    if (typeof entry === "string")
      return <span key={index}> {replacePlaceholdersJsx(entry)}</span>;
    ...
    if (isEntryListItem(entry))
      return <Fragment key={index}>{styleEntryListItem(entry)}</Fragment>;
  }

  function styleEntryListItem(
    entryListItem: EntryListItem,
    index?: number,
  ): ReactNode {
    return (
      <div key={entryListItem.name ?? index} className="pl-6 -indent-6">
        {entryListItem.name && (
          <span className="font-bold">{entryListItem.name}.</span>
        )}
    ...
    return (
      <div key={monsterFeature.name} className="text-xs text-left text-black">
        <span className="italic font-bold">
          {replacePlaceholdersJsx(monsterFeature.name)}.
        </span>
  ...
  function styleSpell(
    ...
    return (
      <div className="grid gap-2 text-xs text-left text-black pt-2">
        <div>
          <span className="italic font-bold">
            {replacePlaceholdersJsx(monsterSpellCasting.name)}.
          </span>
