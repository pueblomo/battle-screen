...
function styleEntry(entry: string | SpellEntry): ReactNode {
  if (typeof entry === "string")
    return <span>{replacePlaceholdersJsx(entry)}</span>;
    ...
    <>
      {entry.items?.map((item) => (
        <p className="pl-6 -indent-6">
          ...
          {item.entries?.map((mappedEntry) => styleEntry(mappedEntry))}
        </p>
      ))}
      {entry.entries && (
        <p className="pl-6 -indent-6">
          <span className="font-bold italic">{entry.name}.</span>{" "}
          {entry.entries?.map((mappedEntry) => styleEntry(mappedEntry))}
        </p>
      )}
    </>
  );
...
export default function SpellDescription({ name }: SpellDescriptionProps) {
  ...
  return (
    <Popover>
      <PopoverTrigger>
        <span className="text-red-800 underline font-[--font-family-libre-baskerville]">
          {name}
        </span>
      </PopoverTrigger>
      <PopoverContent
        ...
        className="drop-shadow-lg shadow-gray-700 text-xs border-4 border-orange-200 font-[--font-family-libre-baskerville]"
      >
        ...
        <p>
          ...
          {styleTime(spell.time[0])}
        </p>
        <p>
          <span className="font-bold">Range:</span> {styleRange(spell.range)}
        </p>
        <br />
        {spell.entries.map((entry) => (
          <div key={entry.toString()}>{styleEntry(entry)}</div>
        ))}
        <br />
